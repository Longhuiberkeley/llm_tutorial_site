...
// Layer labels
[{x:IX,t:'TOKENS'},{x:H1X,t:'HIDDEN'},{x:H2X,t:'HIDDEN'},{x:OX,t:'PREDICTIONS'}].forEach(function(l){
...
for(var h=0;h<H2Y.length;h++){
svg.appendChild(el('circle',{cx:H2X,cy:H2Y[h],r:H2R,fill:'#ebe8e4',stroke:'#d0ccc6','stroke-width':'1'}));
}
// Output nodes
...
btn.disabled=true;
btn.innerHTML='<span class="material-symbols-outlined text-lg animate-spin">progress_activity</span> Training\u2026';
...
var btn=document.getElementById('nn-autotrain-btn');
btn.disabled=false;
btn.innerHTML='<span class="material-symbols-outlined text-lg">auto_fix_high</span> Auto-Train';
var fb=document.getElementById('nn-feedback');
fb.classList.remove('hidden');
fb.innerHTML='<span class="font-bold">The weights converged!</span> After exploring randomly and then adjusting step by step, the network learned that \u201Cmat\u201D is the best prediction. This is what <strong>backpropagation</strong> does \u2014 automatically \u2014 across billions of weights.';
}
},180);
}
/* ── Init ── */
